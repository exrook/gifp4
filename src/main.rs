...
fn cdn_suffix(url: &str) -> Option<&str> {
    url.split_once("discordapp.com/attachments/")
        .map(|(_, s)| s)
}
...
async fn serve(db: &Db, req: Request<Body>) -> Result<Response<MyBody>, hyper::Error> {
    let id = req.uri().path().strip_prefix("/").and_then(parse_id);
...
async fn submit_request(db: &Db, req: Request<Body>) -> Result<Response<MyBody>, hyper::Error> {
    let route = warp::body::content_length_limit(1024).and(warp::body::form()).map(|form: HashMap<String, String>| {
        if let Some((gif, mp4)) = form.get("gif").and_then(|s|cdn_suffix(s)).and_then(|gif|form.get("mp4").and_then(|s|cdn_suffix(s)).map(|mp4|(gif, mp4))) {
            match insert(db, gif, mp4) {
                Ok(id) => {
                    let uri = Uri::builder().path_and_query(format!("/{}", format_id(&id))).build().unwrap();
                    warp::redirect::see_other(uri).into_response()
                }
                Err(e) => {
                    eprintln!("Error inserting: {:?}", e);
                    warp::reply::with_status(warp::reply::reply(), warp::http::StatusCode::INTERNAL_SERVER_ERROR).into_response()
                }
            }
        } else {
    ...
    Ok(Response::from_parts(parts, BoxBody::new(body.map_data(|d|Box::new(d) as Box<dyn Buf + 'static + Send + Sync>))))
}
...
static P3: &'static str = &r#""><meta property="og:video:height" content="202"><meta property="og:video:width" content="250"></head><body><a href="/">Submit</a></body></html>"#;
...
async fn generate_response(db: &Db, id: &ID) -> Result<Response<MyBody>, ()> {
    ...
    Ok(Response::new(MyBody::new(Full::new(
        Box::new(make_page_response(gif, mp4)) as Box<_>,
    ).map_err(|_|todo!()))))
}
...
fn send_home() -> Result<Response<MyBody>, hyper::Error> {
    Ok(Response::new(BoxBody::new(Full::new(
        Box::new(HOME_PAGE.as_bytes()) as _,
    ).map_err(|_|todo!()))))
}
...
fn not_found() -> Result<Response<MyBody>, hyper::Error> {
    let mut resp = Response::new(BoxBody::new(Full::new(Box::new(NOT_FOUND.as_bytes()) as _).map_err(|_|todo!())));
